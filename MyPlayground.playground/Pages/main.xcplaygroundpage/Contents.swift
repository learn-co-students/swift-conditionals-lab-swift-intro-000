...
func isForceWith(name: String) -> Bool{
    ...
    let peopleWithTheForce = ["Luke","Leia","Anakin","Obi Wan","Yoda","Vader"]
    if peopleWithTheForce.contains(name) {
        return true
    }else {
        return false
    }
...
*/
func transferFundsFromAccount(mainAccountBalance: Int, savingsAccountBalance: Int){
    
    if savingsAccountBalance >= 10 {
        mainAccountBalance + 10
        savingsAccountBalance - 10
    }
}
/*:
