...
*/
func isGreater(apples: Int, oranges: Int) -> Bool {
    if(apples > oranges) {
        return true
    } else {
        return false
...
isGreater(apples: 12, oranges: 13) // Returns false
isGreater(apples: 200, oranges: 55) // Returns true
/*: Question 11
### 11. Create a function where the two arguments represent different bank account (one of those bank accounts is yours). What type should these arguments be if we are to then perform some math operations on them? Setup a conditional that will add 10 to your funds and minus 10 from the other funds (bank account) if the other account won't go negative if we were to take 10 dollars from it.
*/
func crazyStuff(myAccount: Double, anotherAccount: Double) {
    if((anotherAccount - 10) <= 0) {
        return
    } else {
        myAccount + 10
        anotherAccount - 10
...
crazyStuff(myAccount: 20, anotherAccount: 10) // my: 30, another: 0
/*:
