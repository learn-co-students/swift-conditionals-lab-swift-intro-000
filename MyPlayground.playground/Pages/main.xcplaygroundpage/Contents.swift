...
func greaterThanB (num: Int) {
    if num >= b {
        print(num)
    }
}

greaterThanB(num: Int(x))
greaterThanB(num: y)
greaterThanB(num: a)
greaterThanB(num: b)

print(Int(x) >= b, y >= b, a >= b, b >= b)
/*: Question 2
...
isGreater(firstInt: 6, secondInt: 2)
...
func isForceWith (name: String) -> Bool {
    if (name == "Luke" || name == "Leia" || name == "Anakin" || name == "Obi Wan") {
        return true
    }
...
*/
// write your code here
func bankBalance(myBalance: Double, otherBalance: Double) {
