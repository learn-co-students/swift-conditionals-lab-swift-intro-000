...
*/
var bx: Bool = Double(a) / Double(b) > x
if bx {
    print(bx)
}
...
*/
var yx: Bool = (Double(y) / Double(x)) > 3
print(yx)
...
*/
if Double(y) > x && ((Double(a) / Double(b)) > 9) {
    print("true")
}
...
func isForceWith(person: String) -> Bool {
    var peopleWithForce = ["Luke", "Leia", "Anakin", "Obi Wan", "Yoda", "Vader"]
    return peopleWithForce.contains(person)
}
...
*/
func bankAccount(account1: Double, account2: Double) {
    if (account2 >= 10) {
        account1 + 10
        account2 - 10
    }
