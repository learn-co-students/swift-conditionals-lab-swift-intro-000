...
func isForceWith(character: String) -> Bool {
    if (character == "Luke" || character == "Leia" || character == "Anakin" || character == "Obi Wan" || character == "Yoda" || character == "Vader") {
        return true
    } else {
        return false
    }
}
print(isForceWith(character: "Yoda"))
...
*/
// write your code here
func manageAccounts(yourAccount: Double, otherAccount: Double)  {
    var newYour = yourAccount
    var newOther = otherAccount
    if (newOther >= 0) {
        newYour += 10
        newOther -= 10
    }
    print("Before exit yourAccount = \(newYour) and otherAccount = \(newOther)")
}
manageAccounts(yourAccount: 50, otherAccount: -1)
