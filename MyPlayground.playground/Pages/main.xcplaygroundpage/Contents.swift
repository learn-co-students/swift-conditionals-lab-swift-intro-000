...
func isForceWith(x: String) -> Bool{
    if ["Luke", "Leia", "Anakin", "Obi Wan", "Yoda", "Vader"].contains(x){
        return true
    } else{
        return false
    }
...
func bankTransfer(myAccount: Int, otherAccount: Int) -> (Int, Int){
    if otherAccount >= 10 {
        return (myAccount + 10, otherAccount - 10)
    } else{
        return (0, 0)
    }
}
