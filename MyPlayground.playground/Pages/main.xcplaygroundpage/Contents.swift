...
*/
// write your code here
func isGreater(_ no1: Int,_ no2: Int) -> Bool {
    if no1 > no2 {
        return true
    } else {
        return false
...
func isForceWith(name: String) -> Bool {
    let namesWithForce = ["Luke", "Leia", "Anakin", "Obi Wan", "Yoda", "Vader"]
    if namesWithForce.contains(name) {
        return true
    } else {
        return false
    }
...
isForceWith(name: "Beyonce")
/*: Question 11
### 11. Create a function where the two arguments represent different bank account (one of those bank accounts is yours). What type should these arguments be if we are to then perform some math operations on them? Setup a conditional that will add 10 to your funds and minus 10 from the other funds (bank account) if the other account won't go negative if we were to take 10 dollars from it.
*/
// write your code here
func transferTen(accountBalanceA: Int, accountBalanceB: Int) {
    if accountBalanceA >= 10 {
        accountBalanceA - 10
        accountBalanceB + 10
    }
}

transferTen(accountBalanceA: 20, accountBalanceB: 30)
