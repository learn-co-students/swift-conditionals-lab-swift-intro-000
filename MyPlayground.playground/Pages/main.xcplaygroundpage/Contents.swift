...
isGreater(firstNumber: 5, secondNumber: 4)
isGreater(firstNumber: 2, secondNumber: 3)
...
*/
func isForceWith(forceUser: String) -> Bool {
    if (forceUser ==  "Luke" || forceUser == "Leia" || forceUser == "Anakin" || forceUser == "Obi Wan" || forceUser == "Yoda" || forceUser == "Vader") {
        return true;
    } else {
        return false;
    }
...
isForceWith(forceUser:"Luke")
isForceWith(forceUser:"Gandalf")
/*: Question 11
### 11. Create a function where the two arguments represent different bank account (one of those bank accounts is yours). What type should these arguments be if we are to then perform some math operations on them? Setup a conditional that will add 10 to your funds and minus 10 from the other funds (bank account) if the other account won't go negative if we were to take 10 dollars from it.
*/
func fundCalc(myAccount: Double, yourAccount: Double){
    if (yourAccount-10>=0){
        myAccount+10;
        yourAccount-10;
    }
