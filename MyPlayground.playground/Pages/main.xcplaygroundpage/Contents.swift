...
*/
func isGreater(_ firstInt:Int, _ secondInt:Int) -> Bool {
    return (firstInt > secondInt)
}

print("isGreater 12 3")
print(isGreater(12,3))
...
*/
func isForceWith(aName:String) -> Bool {
    var forceBoo = false;
    if (aName == "Luke") {
        forceBoo=true
    }
...
print (isForceWith(aName:"Obi Wan"))
print (isForceWith(aName:"ss"))

/*: Question 11
### 11. Create a function where the two arguments represent different bank account (one of those bank accounts is yours). What type should these arguments be if we are to then perform some math operations on them? Setup a conditional that will add 10 to your funds and minus 10 from the other funds (bank account) if the other account won't go negative if we were to take 10 dollars from it.
*/
func transfer10(account1:Double, account2:Double) {
    if (account1 >= 10) {
        account1 - 10
        account2 + 10
    }
...
print ("My \(myAcc) yours \(yourAcc)")

transfer10(account1:myAcc,account2:yourAcc);
print ("My \(myAcc) yours \(yourAcc)")
